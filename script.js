...
// ===== VEDANA & SENSE CONTACT (เวทนากับการกระทบอารมณ์) =====
// The six sense doors. Each door has 3 feeling buttons: sukh / dukkh / neutral.
const CONTACT_SENSES = ['eye', 'ear', 'nose', 'tongue', 'body', 'mind'];
const CONTACT_EMOS = ['sukh', 'dukkh', 'neutral'];

const contactCounts = {};
CONTACT_SENSES.forEach(sense => {
    CONTACT_EMOS.forEach(emo => {
        contactCounts[`contact-${sense}-${emo}`] = 0;
    });
});
...
function showSummary() {
    ...
    // Set Vedana & Sense Contact scores
    renderContactSummary();

    // Set Kilesa family scores
    renderKilesaSummary();
...
function closeSummary() {
    ...
    // Reset Body Breath (Kayagatasati)
...
/**
 * Build the "Kilesa families" rows inside the summary modal.
 * Only pairs that were actually tapped are listed, so the summary stays readable.
 */
function renderKilesaSummary() {
    const list = document.getElementById('sumKilesaList');
    const category = document.getElementById('sumKilesaCategory');
    if (!list || !category) return;

    const dict = langDict[currentLang] || {};
    const scoreClass = {
        lobha: ['score-lobha', 'score-alobha'],
        dosa: ['score-dosa', 'score-adosa'],
        moha: ['score-moha', 'score-amoha']
    };

    let html = '';
    KILESA_FAMILIES.forEach(fam => {
        const famKey = fam.charAt(0).toUpperCase() + fam.slice(1);
        let rows = '';

        for (let i = 1; i <= KILESA_PAIR_COUNT[fam]; i++) {
            const a = kilesaCounts[`kilesa-${fam}-${i}a`];
            const b = kilesaCounts[`kilesa-${fam}-${i}b`];
            if (!a && !b) continue; // untouched pair

            rows += `
            <div class="summary-kilesa-row">
                <span class="summary-score-item ${scoreClass[fam][0]}">
                    <span class="score-label">${dict[`kilesa${famKey}${i}a`] || ''}</span>
                    <span class="score-value">${a}</span>
                </span>
                <span class="summary-score-item ${scoreClass[fam][1]}">
                    <span class="score-label">${dict[`kilesa${famKey}${i}b`] || ''}</span>
        ...
        if (rows) {
            html += `
        <div class="summary-kilesa-family">
            <h4 class="summary-kilesa-family-title">${dict[`kilesa${famKey}Title`] || ''}</h4>
            ${rows}
        </div>`;
        }
    });
    ...
    list.innerHTML = html;
    category.style.display = html ? 'block' : 'none';
}

/**
 * Build the "Vedana & Sense Contact" rows inside the summary modal.
 * Rendered from state so labels always follow the current language.
 */
function renderContactSummary() {
    const list = document.getElementById('sumContactList');
    if (!list) return;

    const dict = langDict[currentLang] || {};

    list.innerHTML = CONTACT_SENSES.map(sense => {
        const key = sense.charAt(0).toUpperCase() + sense.slice(1);
        const label = dict[`contact${key}Short`] || sense;
        const scores = CONTACT_EMOS.map(emo => `
            <span class="summary-score-item score-${emo}">
                <span class="score-label">${dict[emo === 'dukkh' ? 'dukkh' : emo] || emo}</span>
                <span class="score-value">${contactCounts[`contact-${sense}-${emo}`]}</span>
            </span>`).join('');

        return `
        <div class="summary-contact-row">
            <div class="summary-contact-label">${label}</div>
            <div class="summary-contact-values">${scores}</div>
        </div>`;
    }).join('');
}
...
function updateSummaryLanguage() {
    ...
    const sumKilesaTitle = document.getElementById('sumKilesaTitle');
    if (sumKilesaTitle) sumKilesaTitle.textContent = dict.kilesaSectionTitle;
    renderKilesaSummary();
    const sumContactTitle = document.getElementById('sumContactTitle');
    if (sumContactTitle) sumContactTitle.textContent = dict.contactSectionTitle;
    renderContactSummary();
...
/**
 * Handle sense-contact sensation button clicks
 * @param {string} id - The specific html ID of the button (e.g. 'contact-eye-sukh')
 */
function recordContact(id) {
    if (!(id in contactCounts)) return;
    ...
    const counterEl = document.getElementById(`count-${id}`);
    if (counterEl) {
        counterEl.innerText = contactCounts[id];
        counterEl.classList.remove('pop');
        void counterEl.offsetWidth; // trigger reflow
        counterEl.classList.add('pop');
    }

    // 3. Pop animation on the button
    const btn = document.getElementById(id);
    if (btn) {
        btn.classList.remove('pop');
        void btn.offsetWidth;
...
/**
 * Reset every sense-contact counter
 */
function resetContactGroup() {
    Object.keys(contactCounts).forEach(key => {
        contactCounts[key] = 0;
...
// ===== UNIFIED TAP HANDLER FOR COUNTING BUTTONS =====
//
// A tap is only counted on touchend, and only when the finger neither moved
// far from where it landed nor lifted outside the button. That keeps the page
// scrollable when a scroll gesture happens to start on a button, and stops a
// slightly-shifted finger from losing the tap.
//
// touchend calls preventDefault() so the browser does not follow up with a
// synthetic click; the capture-phase guard below swallows any that still slip
// through, so a single tap can never be counted twice.
//
// Mouse and keyboard still go through each button's own inline onclick.

const TAP_MOVE_TOLERANCE = 14; // px of finger travel still treated as a tap
const GHOST_CLICK_WINDOW = 700; // ms after a touch-fired tap that clicks are ignored

let lastTouchFireTime = 0;

/**
 * Run a button's counting action (its inline onclick) from the touch path.
 */
function fireButtonAction(btn, event) {
    if (typeof btn.onclick === 'function') {
        btn.onclick.call(btn, event);
    }
    lastTouchFireTime = Date.now();
}

/**
 * Swallow the synthetic click that some browsers still emit after a tap.
 * Runs during the capture phase on document, i.e. before the button's own
 * inline onclick, so the count cannot be duplicated.
 */
document.addEventListener('click', event => {
    if (Date.now() - lastTouchFireTime >= GHOST_CLICK_WINDOW) return;
    const target = event.target instanceof Element
        ? event.target.closest('.circle-btn, .mind-input-btn')
        : null;
    if (!target) return;
...
/**
 * Wire up one counting button.
 * @param {Element} btn
 * @param {function(Event): boolean} [shouldSkip] - return true to let the
 *        browser handle the touch natively (e.g. a text field inside the button)
 */
function attachTapHandler(btn, shouldSkip) {
    let litTimeout = null;
    let tracking = false;
    let startX = 0;
    let startY = 0;
    ...
    const lightOn = () => {
        ...
        if (!btn.classList.contains('disabled-input')) btn.classList.add('lit');
    };

    const lightOff = delay => {
        if (litTimeout) { clearTimeout(litTimeout); litTimeout = null; }
    ...
    btn.addEventListener('touchstart', event => {
        if (shouldSkip && shouldSkip(event)) { tracking = false; return; }
    ...
    btn.addEventListener('touchmove', event => {
        ...
        const touch = event.touches[0];
        if (!touch) return;
        if (Math.abs(touch.clientX - startX) > TAP_MOVE_TOLERANCE ||
            Math.abs(touch.clientY - startY) > TAP_MOVE_TOLERANCE) {
            // The finger is scrolling, not tapping.
            tracking = false;
            lightOff(0);
        }
    }, { passive: true });
    ...
    btn.addEventListener('touchend', event => {
        if (!tracking) { lightOff(0); return; }
        tracking = false;

        const touch = event.changedTouches[0];
        if (touch) {
            const box = btn.getBoundingClientRect();
            const outside = touch.clientX < box.left - TAP_MOVE_TOLERANCE ||
                touch.clientX > box.right + TAP_MOVE_TOLERANCE ||
                touch.clientY < box.top - TAP_MOVE_TOLERANCE ||
                touch.clientY > box.bottom + TAP_MOVE_TOLERANCE;
            if (outside) { lightOff(0); return; } // lifted off the button
        }

        event.preventDefault(); // no synthetic click after this tap
        fireButtonAction(btn, event);
        lightOff(600);
    }, { passive: false });
    ...
    // --- MOUSE: glow only; counting stays with the inline onclick ---
    btn.addEventListener('mousedown', lightOn);
    btn.addEventListener('mouseup', () => lightOff(600));
    btn.addEventListener('mouseleave', () => {
        if (btn.classList.contains('lit')) lightOff(300);
    });
}

document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.circle-btn').forEach(btn => attachTapHandler(btn));

    // Mind input buttons are wrapper divs holding an editable field, so a touch
    // on the field itself (or on its clear button) must be left alone.
    document.querySelectorAll('.mind-input-btn').forEach(wrapper => {
        attachTapHandler(wrapper, event => {
            const field = wrapper.querySelector('.mind-input-field');
            if (field && field.contains(event.target) &&
                field.getAttribute('contenteditable') === 'true') {
                return true;
            }
            const clearBtn = wrapper.querySelector('.mind-input-clear');
            return !!(clearBtn && clearBtn.contains(event.target));
        });
    });
});
...
const langDict = {
    'th': {
        siteTitle: 'แอปเจริญสติ ฝึกสติ สมาธิ | Araro IU Mindfulness & Meditation App',
        siteDesc: 'แอปเจริญสติสมาธิตามหลักสติปัฏฐาน 4 สำหรับสังเกตสภาวะในชีวิตประจำวัน',
        ...
        menTitle: 'เวทนาทางใจ',
        // Mind sub-page tabs
        tabMindPairs: 'ดูจิตเป็นคู่',
        tabMindKilesa: 'กิเลสแต่ละตระกูล',
        mindPairsSectionTitle: 'ดูจิตเป็นคู่',
        kilesaSectionTitle: 'กิเลสแต่ละตระกูล',
        kilesaSectionDesc: 'สังเกตกิเลสที่เกิดขึ้นในแต่ละขณะ ว่าเป็นตระกูลไหน แล้วมีสติรู้ทันตามความเป็นจริง',
        kilesaLobhaTitle: 'กิเลสตระกูลโลภะ',
        kilesaLobha1a: 'มีราคะ',
        kilesaLobha1b: 'ไม่มีราคะ',
        kilesaLobha2a: 'อยาก',
        kilesaLobha2b: 'ไม่อยาก',
        kilesaLobha3a: 'อยากได้',
        kilesaLobha3b: 'ไม่อยากได้',
        kilesaLobha4a: 'อยากรักษาไว้',
        kilesaLobha4b: 'ไม่อยากรักษาไว้',
        kilesaLobha5a: 'ไม่สดชื่น',
        kilesaLobha5b: 'สดชื่น',
        kilesaDosaTitle: 'กิเลสตระกูลโทสะ',
        kilesaDosa1a: 'เบื่อ',
        kilesaDosa1b: 'ไม่เบื่อ',
        kilesaDosa2a: 'กลัว',
        kilesaDosa2b: 'ไม่กลัว',
        kilesaDosa3a: 'หงุดหงิด',
        kilesaDosa3b: 'ไม่หงุดหงิด',
        kilesaDosa4a: 'เซ็ง',
        kilesaDosa4b: 'ไม่เซ็ง',
        kilesaDosa5a: 'อิจฉา',
        kilesaDosa5b: 'ไม่อิจฉา',
        kilesaDosa6a: 'ขี้เกียจ',
        kilesaDosa6b: 'ไม่ขี้เกียจ',
        kilesaDosa7a: 'วิตกกังวล',
        kilesaDosa7b: 'ไม่วิตกกังวล',
        kilesaDosa8a: 'รำคาญใจ',
        kilesaDosa8b: 'ไม่รำคาญใจ',
        kilesaDosa9a: 'หดหู่',
        kilesaDosa9b: 'ไม่หดหู่',
        kilesaDosa10a: 'ไม่แช่มชื่นใจ',
        kilesaDosa10b: 'แช่มชื่นใจ',
        kilesaDosa11a: 'ไม่พอใจ',
        kilesaDosa11b: 'พอใจ',
        kilesaDosa12a: 'ขัดใจ',
        kilesaDosa12b: 'ไม่ขัดใจ',
        kilesaDosa13a: 'เคืองใจ',
        kilesaDosa13b: 'ไม่เคืองใจ',
        kilesaDosa14a: 'ร้อนใจ',
        kilesaDosa14b: 'ไม่ร้อนใจ',
        kilesaDosa15a: 'ตกใจ',
        kilesaDosa15b: 'ไม่ตกใจ',
        kilesaDosa16a: 'ขุ่นเคือง',
        kilesaDosa16b: 'ไม่ขุ่นเคือง',
        kilesaDosa17a: 'เสียใจ',
        kilesaDosa17b: 'ไม่เสียใจ',
        kilesaMohaTitle: 'กิเลสตระกูลโมหะ',
        kilesaMoha1a: 'เผลอ',
        kilesaMoha1b: 'ไม่เผลอ',
        kilesaMoha2a: 'ฟุ้งซ่าน',
        kilesaMoha2b: 'ไม่ฟุ้งซ่าน',
        kilesaMoha3a: 'ซึม',
        kilesaMoha3b: 'ไม่ซึม',
        kilesaMoha4a: 'ใจลอย',
        kilesaMoha4b: 'ไม่ใจลอย',
        kilesaMoha5a: 'ลังเล',
        kilesaMoha5b: 'ไม่ลังเล',
        kilesaMoha6a: 'สงสัย',
        kilesaMoha6b: 'ไม่สงสัย',
        // Vedana sub-page tabs
        tabVedana5: 'เวทนา 5',
        tabVedanaContact: 'เวทนากับการกระทบอารมณ์',
        vedana5SectionTitle: 'เวทนา 5',
        // Vedana & Sense Contact
        contactSectionTitle: 'เวทนากับการกระทบอารมณ์',
        contactSectionDesc: 'เมื่อมีการกระทบทางทวารทั้ง 6 จิตใจมีความสุข ทุกข์ หรือ เฉยๆ ให้มีสติรู้ทัน ณ ขณะนั้น',
        contactEyeText: 'เมื่อ <strong>ตา</strong> มองเห็น <strong>รูป</strong>',
        contactEarText: 'เมื่อ <strong>หู</strong> ได้ยิน <strong>เสียง</strong>',
        contactNoseText: 'เมื่อ <strong>จมูก</strong> ได้กลิ่น <strong>กลิ่น</strong>',
        contactTongueText: 'เมื่อ <strong>ลิ้น</strong> ลิ้ม <strong>รส</strong>',
        contactBodyText: 'เมื่อ <strong>กาย</strong> สัมผัส <strong>สิ่งที่มากระทบ</strong>',
        contactMindText: 'เมื่อ <strong>จิตใจ</strong> คิด นึก <strong>เรื่องราวต่างๆ</strong>',
        contactEyeShort: 'ตา เห็น รูป',
        contactEarShort: 'หู ได้ยิน เสียง',
        contactNoseShort: 'จมูก ได้กลิ่น',
        contactTongueShort: 'ลิ้น ลิ้มรส',
        contactBodyShort: 'กาย สัมผัส',
        contactMindShort: 'ใจ คิดนึก',
        sukh: 'สุข',
        dukkh: 'ทุกข์',
        neutral: 'เฉยๆ',
        reset: 'รีเซ็ต',
        ...
        navMind: 'จิต',
        navDhamma: 'ธรรม',
        footerLink: 'วิธีการเล่นแอปเจริญสติสังเกตเวทนา',
        search: 'ค้นหา',
        ...
        homeIntroText2: 'การเจริญสติ จะสังเกตการเจริญสติตามจริตนิสัย ลองใช้แอปนี้เป็นเครื่องทดสอบของสติว่า ถูกจริตนิสัยกับกรรมฐานกองไหน กาย หรือ เวทนา หรือ จิต หรือ ธรรม',
        homeFooterNoteHtml: '',
        ...
        guideLiberationText: 'แอปเจริญสตินี้ ก็เปรียบเสมือนเครื่องมือที่ช่วยในการเจริญสติ ช่วยให้มีการระลึกถึงสติ ในเบื้องต้น เพราะขณะที่กดปุ่ม มีการนับจำนวนเวทนาแต่ละครั้ง นั้นก็เท่ากับว่า ขณะนั้นมีสติ เท่ากับจำนวนที่กดปุ่มลงไปนั้น',
        ...
        // Kayagatasati
        bodyBreathTitle: 'กายคตาสติ',
        tabBodyPosture: 'อิริยาบถ 4',
        tabBodyBreath: 'กายคตาสติ',
        ...
        bodyGuideFinalText: 'แอปเจริญสตินี้ ก็เปรียบเสมือนเครื่องมือที่ช่วยในการเจริญสติ ช่วยให้มีการระลึกถึงสติ ในเบื้องต้น เพราะขณะที่กดปุ่ม มีการนับจำนวนอริยาบทกายแต่ละครั้ง นั้นก็เท่ากับว่า ขณะนั้นมีสติ เท่ากับจำนวนที่กดปุ่มลงไปนั้น',
        ...
        mindGuideFinalText: 'แอปเจริญสตินี้ ก็เปรียบเสมือนเครื่องมือที่ช่วยในการเจริญสติ ช่วยให้มีการระลึกถึงสติ ในเบื้องต้น เพราะขณะที่กดปุ่ม มีการนับจำนวนจิตแต่ละครั้ง นั้นก็เท่ากับว่า ขณะนั้นมีสติ เท่ากับจำนวนที่กดปุ่มลงไปนั้น',
        ...
        dhammaGuideFinalText: 'แอปเจริญสตินี้ เปรียบเสมือนเครื่องมือที่ช่วยในการเจริญสติ ช่วยให้มีการระลึกถึงสติ ในเบื้องต้น เพราะขณะที่กดปุ่ม มีการนับจำนวนแต่ละครั้ง นั้นก็เท่ากับว่า ขณะนั้นมีสติ เท่ากับจำนวนที่กดปุ่มลงไปนั้น'
    },
    'en': {
        siteTitle: 'Araro IU Mindfulness & Meditation | Vipassana App',
        siteDesc: 'Practicing mindfulness and meditation step-by-step through Satipatthana principles (Body, Feelings, Mind, and Dhamma) in daily life.',
        ...
        // Mind sub-page tabs
        tabMindPairs: 'The Mind in Pairs',
        tabMindKilesa: 'Families of Defilements',
        mindPairsSectionTitle: 'The Mind in Pairs',
        kilesaSectionTitle: 'Families of Defilements',
        kilesaSectionDesc: 'Notice which family a defilement belongs to as it arises, and be mindfully aware of it as it truly is.',
        kilesaLobhaTitle: 'The Lobha Family (Greed)',
        kilesaLobha1a: 'With lust',
        kilesaLobha1b: 'Without lust',
        kilesaLobha2a: 'Craving',
        kilesaLobha2b: 'Not craving',
        kilesaLobha3a: 'Wanting to get',
        kilesaLobha3b: 'Not wanting to get',
        kilesaLobha4a: 'Wanting to keep',
        kilesaLobha4b: 'Not wanting to keep',
        kilesaLobha5a: 'Unrefreshed',
        kilesaLobha5b: 'Refreshed',
        kilesaDosaTitle: 'The Dosa Family (Aversion)',
        kilesaDosa1a: 'Bored',
        kilesaDosa1b: 'Not bored',
        kilesaDosa2a: 'Afraid',
        kilesaDosa2b: 'Not afraid',
        kilesaDosa3a: 'Irritated',
        kilesaDosa3b: 'Not irritated',
        kilesaDosa4a: 'Fed up',
        kilesaDosa4b: 'Not fed up',
        kilesaDosa5a: 'Envious',
        kilesaDosa5b: 'Not envious',
        kilesaDosa6a: 'Lazy',
        kilesaDosa6b: 'Not lazy',
        kilesaDosa7a: 'Anxious',
        kilesaDosa7b: 'Not anxious',
        kilesaDosa8a: 'Annoyed',
        kilesaDosa8b: 'Not annoyed',
        kilesaDosa9a: 'Gloomy',
        kilesaDosa9b: 'Not gloomy',
        kilesaDosa10a: 'Not cheerful',
        kilesaDosa10b: 'Cheerful',
        kilesaDosa11a: 'Dissatisfied',
        kilesaDosa11b: 'Satisfied',
        kilesaDosa12a: 'Frustrated',
        kilesaDosa12b: 'Not frustrated',
        kilesaDosa13a: 'Offended',
        kilesaDosa13b: 'Not offended',
        kilesaDosa14a: 'Troubled',
        kilesaDosa14b: 'Not troubled',
        kilesaDosa15a: 'Startled',
        kilesaDosa15b: 'Not startled',
        kilesaDosa16a: 'Resentful',
        kilesaDosa16b: 'Not resentful',
        kilesaDosa17a: 'Sorrowful',
        kilesaDosa17b: 'Not sorrowful',
        kilesaMohaTitle: 'The Moha Family (Delusion)',
        kilesaMoha1a: 'Heedless',
        kilesaMoha1b: 'Not heedless',
        kilesaMoha2a: 'Restless',
        kilesaMoha2b: 'Not restless',
        kilesaMoha3a: 'Dull',
        kilesaMoha3b: 'Not dull',
        kilesaMoha4a: 'Absent-minded',
        kilesaMoha4b: 'Not absent-minded',
        kilesaMoha5a: 'Hesitant',
        kilesaMoha5b: 'Not hesitant',
        kilesaMoha6a: 'Doubtful',
        kilesaMoha6b: 'Not doubtful',
        // Vedana sub-page tabs
        tabVedana5: 'The Five Vedana',
        tabVedanaContact: 'Vedana and Sense Contact',
        vedana5SectionTitle: 'The Five Vedana',
        // Vedana & Sense Contact
        contactSectionTitle: 'Vedana and Sense Contact',
        contactSectionDesc: 'Whenever contact occurs at one of the six sense doors, the mind feels happy, suffering, or neutral — be mindful and aware of it at that moment.',
        contactEyeText: 'When the <strong>eye</strong> sees a <strong>form</strong>',
        contactEarText: 'When the <strong>ear</strong> hears a <strong>sound</strong>',
        contactNoseText: 'When the <strong>nose</strong> smells an <strong>odour</strong>',
        contactTongueText: 'When the <strong>tongue</strong> tastes a <strong>flavour</strong>',
        contactBodyText: 'When the <strong>body</strong> feels a <strong>touch</strong>',
        contactMindText: 'When the <strong>mind</strong> thinks of <strong>mental objects</strong>',
        contactEyeShort: 'Eye · Form',
        contactEarShort: 'Ear · Sound',
        contactNoseShort: 'Nose · Odour',
        contactTongueShort: 'Tongue · Flavour',
        contactBodyShort: 'Body · Touch',
        contactMindShort: 'Mind · Thought',
        sukh: 'Happy',
        dukkh: 'Suffering',
        neutral: 'Neutral',
        reset: 'Reset',
        ...
        // Kayagatasati
        bodyBreathTitle: 'Kayagatasati',
        tabBodyPosture: '4 Postures',
        tabBodyBreath: 'Kayagatasati',
        ...
        sumBodyBreathTitle: 'Kayagatasati',
...
function updateLanguageUI() {
    ...
    // Update Browser Document Title & Meta Description dynamically
    if (dict.siteTitle) {
        document.title = dict.siteTitle;
    }
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc && dict.siteDesc) {
        metaDesc.setAttribute('content', dict.siteDesc);
    }
    ...
    // Vedana sub-page tabs
    const tabV5 = document.getElementById('tabVedana5');
    if (tabV5) tabV5.innerText = dict.tabVedana5;
    const tabVContact = document.getElementById('tabVedanaContact');
    if (tabVContact) tabVContact.innerText = dict.tabVedanaContact;
    const v5Title = document.getElementById('vedana5SectionTitle');
    if (v5Title) v5Title.innerText = dict.vedana5SectionTitle;

    // Vedana & Sense Contact section
    const contactTitle = document.getElementById('contactSectionTitle');
    if (contactTitle) contactTitle.innerText = dict.contactSectionTitle;
    const contactDesc = document.getElementById('contactSectionDesc');
    if (contactDesc) contactDesc.innerText = dict.contactSectionDesc;
    CONTACT_SENSES.forEach(sense => {
        const key = sense.charAt(0).toUpperCase() + sense.slice(1);
        const el = document.getElementById(`contact${key}Text`);
        if (el) el.innerHTML = dict[`contact${key}Text`];
    });
    ...
    const nDhamma = document.getElementById('navDhamma');
    if(nDhamma) nDhamma.innerText = dict.navDhamma;
    ...
    const cardDhammaT = document.getElementById('cardDhammaTitle');
    if(cardDhammaT) cardDhammaT.innerText = dict.navDhamma;
    ...
    const igFollowEl = document.getElementById('igFollowText');
    if(igFollowEl) igFollowEl.innerText = dict.igFollowText;
    ...
    // ===== Mind sub-page tabs & Kilesa families =====
    const tabMPairs = document.getElementById('tabMindPairs');
    if (tabMPairs) tabMPairs.innerText = dict.tabMindPairs;
    const tabMKilesa = document.getElementById('tabMindKilesa');
    if (tabMKilesa) tabMKilesa.innerText = dict.tabMindKilesa;
    const mindPairsTitle = document.getElementById('mindPairsSectionTitle');
    if (mindPairsTitle) mindPairsTitle.innerText = dict.mindPairsSectionTitle;
    const kilesaTitle = document.getElementById('kilesaSectionTitle');
    if (kilesaTitle) kilesaTitle.innerText = dict.kilesaSectionTitle;
    const kilesaDesc = document.getElementById('kilesaSectionDesc');
    if (kilesaDesc) kilesaDesc.innerText = dict.kilesaSectionDesc;

    KILESA_FAMILIES.forEach(fam => {
        const famKey = fam.charAt(0).toUpperCase() + fam.slice(1);
        const famTitle = document.getElementById(`kilesa${famKey}Title`);
        if (famTitle) famTitle.innerText = dict[`kilesa${famKey}Title`];

        for (let i = 1; i <= KILESA_PAIR_COUNT[fam]; i++) {
            ['a', 'b'].forEach(side => {
                const btn = document.getElementById(`kilesa-${fam}-${i}${side}`);
                if (btn) btn.innerText = dict[`kilesa${famKey}${i}${side}`];
            });
        }
    });
...
// Body breath (Kayagatasati) counters
...
// ===== KILESA FAMILIES (กิเลสแต่ละตระกูล) =====
// Each family holds a list of opposing pairs. Button ids follow
// `kilesa-<family>-<n><a|b>`; 'a' is the first word of the pair as listed.
const KILESA_FAMILIES = ['lobha', 'dosa', 'moha'];
const KILESA_PAIR_COUNT = { lobha: 5, dosa: 17, moha: 6 };

const kilesaCounts = {};
KILESA_FAMILIES.forEach(fam => {
    for (let i = 1; i <= KILESA_PAIR_COUNT[fam]; i++) {
        kilesaCounts[`kilesa-${fam}-${i}a`] = 0;
        kilesaCounts[`kilesa-${fam}-${i}b`] = 0;
    }
});

/**
 * Record a kilesa button click
 */
function recordKilesa(id) {
    if (!(id in kilesaCounts)) return;

    kilesaCounts[id]++;

    const counterEl = document.getElementById(`count-${id}`);
    if (counterEl) {
        counterEl.innerText = kilesaCounts[id];
        counterEl.classList.remove('pop');
        void counterEl.offsetWidth;
        counterEl.classList.add('pop');
    }
...
/**
 * Reset one kilesa family, or every family when called with no argument
 */
function resetKilesaGroup(family) {
    Object.keys(kilesaCounts).forEach(key => {
        if (family && !key.startsWith(`kilesa-${family}-`)) return;
        kilesaCounts[key] = 0;
        const el = document.getElementById(`count-${key}`);
        if (el) el.innerText = '0';
    });
 ...
 * Switch Mind Tabs
 * 'pairs'  -> ดูจิตเป็นคู่
 * 'kilesa' -> กิเลสแต่ละตระกูล
 */
function switchMindTab(tabName) {
    const tabs = {
        pairs: [document.getElementById('tabMindPairs'), document.getElementById('mind-tab-pairs')],
        kilesa: [document.getElementById('tabMindKilesa'), document.getElementById('mind-tab-kilesa')]
    };
    if (!tabs[tabName]) return;
    if (Object.values(tabs).some(([tab, content]) => !tab || !content)) return;

    Object.keys(tabs).forEach(name => {
        const [tab, content] = tabs[name];
...
/**
 * Switch Vedana Tabs
 * 'vedana5'  -> เวทนา 5 (ทางกาย + ทางใจ)
 * 'contact'  -> เวทนากับการกระทบอารมณ์
 */
function switchVedanaTab(tabName) {
    const tabs = {
        vedana5: [document.getElementById('tabVedana5'), document.getElementById('vedana-tab-vedana5')],
        contact: [document.getElementById('tabVedanaContact'), document.getElementById('vedana-tab-contact')]
    };
    if (!tabs[tabName]) return;
    if (Object.values(tabs).some(([tab, content]) => !tab || !content)) return;
